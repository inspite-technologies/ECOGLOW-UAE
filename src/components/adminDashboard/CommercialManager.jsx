...
import { fetchServices, updateService } from '../../services/commercialAPI';
...
const CommercialManager = () => {
    ...
    const loadData = async () => {
      try {
        ...
        let currentList = (data.servicesList || []).map(item => ({
          ...
          // Ensure fields exist for existing data
  ...
  const handleAddCard = () => {
    ...
    setServicesData(prev => ({
      ...prev,
      servicesList: [...prev.servicesList, newCard]
    }));
  };
  ...
  const handleImageUpload = (e, key, itemId = null) => {
    const file = e.target.files[0];
    if (file) {
      const previewUrl = URL.createObjectURL(file);
      if (itemId !== null) {
        const index = servicesData.servicesList.findIndex(item => item.id === itemId);
        if (index > -1) handleCardUpdate(index, 'image', previewUrl);
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      if (selectedFiles['bannerImage']) formData.append('bannerImage', selectedFiles['bannerImage']);
      if (selectedFiles['introSideImage']) formData.append('introSideImage', selectedFiles['introSideImage']);

      servicesData.servicesList.forEach((item, index) => {
  ...
  return (
    <div style={{ padding: '30px', maxWidth: '1400px', margin: '0 auto', fontFamily: 'Inter, sans-serif', backgroundColor: '#f8fafc' }}>
      ...
      <div style={headerBarStyle}>
        <div>
          <h2 style={{ margin: 0, color: '#0f766e' }}>Commercial Master Manager</h2>
          <p style={{ margin: '5px 0 0 0', color: '#64748b' }}>Edit headings, descriptions, and contact links.</p>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 400px', gap: '30px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '30px' }}>
          ...
          <section style={cardStyle}>
            <div style={sectionHeaderStyle}><Layout size={20} color="#14b8a6" /> <h3>Hero & Commercial Intro</h3></div>
              ...
              {servicesData.servicesList.map((service, index) => (
                ...
                >
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px', paddingBottom: '8px', borderBottom: '1px solid #e2e8f0' }}>
                    <div style={{ cursor: 'grab', display: 'flex', alignItems: 'center', color: '#94a3b8' }} title="Drag to reorder">
                      <GripVertical size={18} />
                    </div>
                    <button onClick={() => handleRemoveCard(index)} style={deleteBtnStyle}>
                      <Trash2 size={14} />
                    </button>
                  </div>

                  <div style={{ display: 'flex', gap: '15px', marginBottom: '10px' }}>
                    <div onClick={() => document.getElementById(`card-up-${service.id}`).click()} style={thumbStyle}>
                      {service.image ? (
                        <img src={getImageUrl(service.image)} style={fullImg} alt="service" />
                      ) : (
                        <div style={{ textAlign: 'center' }}><Upload size={18} color="#94a3b8" /></div>
                      )}
                    </div>
                    <input
                      type="file"
                      id={`card-up-${service.id}`}
                      hidden
                      onChange={(e) => handleImageUpload(e, 'servicesList', service.id)}
                    />
                    <div style={{ flex: 1 }}>
                  ...
                  <textarea style={{ ...smallInput, fontSize: '0.8rem', marginBottom: '10px' }} rows="2" value={service.desc} onChange={(e) => handleCardUpdate(index, 'desc', e.target.value)} placeholder="Description..." />

                  {/* --- UPDATED CONTACT FIELDS (2 Columns) --- */}
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', paddingTop: '10px', borderTop: '1px dashed #cbd5e1' }}>
                    ...
                    <div style={{ position: 'relative' }}>
                      ...
                      <input
                        style={{ ...smallInput, paddingLeft: '25px', fontSize: '0.75rem' }}
                        value={service.phoneNumber}
                        onChange={(e) => handleCardUpdate(index, 'phoneNumber', e.target.value)}
                        placeholder="Book Link (Phone No. or URL)"
                      />
                    </div>
                    ...
                    <div style={{ position: 'relative' }}>
                      ...
                      <input
                        ...
                        onChange={(e) => handleCardUpdate(index, 'whatsappNumber', e.target.value)}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '30px' }}>
          <section style={cardStyle}>
            <div style={sectionHeaderStyle}><Monitor size={18} color="#14b8a6" /> <h3>Main Banner</h3></div>
            <div onClick={() => bannerImgRef.current.click()} style={bannerUploadBox}>
              {servicesData.bannerImage ? (
                <img src={getImageUrl(servicesData.bannerImage)} style={fullImg} alt="banner" />
              ) : (
          ...
          <section style={cardStyle}>
            <div style={sectionHeaderStyle}><ImageIcon size={18} color="#14b8a6" /> <h3>Side Image</h3></div>
            <div onClick={() => introImgRef.current.click()} style={bannerUploadBox}>
              {servicesData.introSideImage ? (
                <img src={getImageUrl(servicesData.introSideImage)} style={fullImg} alt="side" />
              ) : (
                <div style={{ textAlign: 'center' }}><Upload color="#94a3b8" /><p style={{ fontSize: '0.8rem' }}>Upload Image</p></div>
              )}
            </div>
          ...
          <section style={cardStyle}>
            <div style={sectionHeaderStyle}><Users size={18} color="#14b8a6" /> <h3>Trust Text</h3></div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
              ...
              <div>
                ...
                <input
                  ...
                  name="contactEmail"
                  style={inputStyle}
                  placeholder="commercial@ecoglow.ae"
              ...
              <div>
                ...
                <input name="newsletterTitle" style={inputStyle} value={servicesData.newsletterTitle} onChange={handleTextChange} placeholder="Title" />
              </div>
              <div>
                <label style={labelStyle}>Newsletter Subtitle</label>
                <textarea name="newsletterSubtitle" style={{ ...inputStyle, marginTop: '10px' }} rows="2" value={servicesData.newsletterSubtitle} onChange={handleTextChange} placeholder="Subtitle" />
              </div>
            </div>
          </section>
        </div>
